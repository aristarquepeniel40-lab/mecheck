...
deliberement EXCLU la coherence `end_date >= start_date` du validator de
la classe — decision documentee en ARCHITECTURE.md §2.4 (“classe de
structure pure”), avec la note : *“cette coherence est deplacee vers une
fonction separee, potentiellement dans mecheck”*. C’est exactement ce
qui est implemente ici, dans
...
(completude, typage).
[`mecheck::run_checks()`](https://aristarquepeniel40-lab.github.io/mecheck/reference/run_checks.md)
verifie les relations ENTRE les objets d’un projet complet — un niveau
au-dessus.
...
- [`default_rules()`](https://aristarquepeniel40-lab.github.io/mecheck/reference/default_rules.md)
  ...
  - `logframe_complet` (reutilise
    [`mecore::me_validate()`](https://rdrr.io/pkg/mecore/man/me_validate.html)
    sur le logframe, mais capture l’erreur au lieu de la laisser
    remonter)
- `run_checks(project, rules = default_rules())` → `me_check_report`,
  **ne leve jamais d’erreur** — produit un constat complet meme si
  ...
  [`mecore::me_validate()`](https://rdrr.io/pkg/mecore/man/me_validate.html),
  qui s’arrete a la premiere violation).
- `has_failures(report)` — TRUE/FALSE rapide pour un usage en pipeline
  (ex. bloquer un `mereport::generate_report()` si
...
`mereport::generate_report()` pourrait appeler
[`mecheck::run_checks()`](https://aristarquepeniel40-lab.github.io/mecheck/reference/run_checks.md)
avant de rendre un rapport, et avertir (ou bloquer) si
[`has_failures()`](https://aristarquepeniel40-lab.github.io/mecheck/reference/has_failures.md)
